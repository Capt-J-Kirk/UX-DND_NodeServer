...
subT  <- sprintf("Confidence interval 95%%, %d Games ~ %d Scenes.", length(unique(dataFrame$gameId)), nrow(dataFrame));
...
## Does distribution of general experience answers, in CONTROL group, follows the null-distribution (H0) or not (HA).
```{r}
...
# Or => There is not statistical significance that the distribution of answers is different from the null-distribution.
```
...
# Or => There IS statistical significance that the distribution of answers is different from the null-distribution.
```
...
dataFrame %>%
  ...
  #scale_fill_manual(values = c( "black", "#EE6A50", "#008B00",  "#EE7600", "#F0FFF0")) +
  scale_fill_manual(values = c( "#EE6A50", "#EE7600", "#008B00", "#F0FFF0", "black"),
...
~group, ~generalExp0_2, ~generalExp1_3, ~generalExp2_4,
  "controlGroup",
  nrow(filter(sceneData, isControlGroup == 1 & generalExp < 3)),
  nrow(filter(sceneData, isControlGroup == 1 & generalExp > 0 & generalExp < 4)),
  nrow(filter(sceneData, isControlGroup == 1 & generalExp > 1)),
  "testGroup",
  nrow(filter(sceneData, isControlGroup == 0 & generalExp < 3)),
  nrow(filter(sceneData, isControlGroup == 0 & generalExp > 0 & generalExp < 4)),
  nrow(filter(sceneData, isControlGroup == 0 & generalExp > 1))
)
...
~group, ~diffExp0_2, ~diffExp1_3, ~diffExp2_4,
  "controlGroup",
  nrow(filter(sceneData, isControlGroup == 1 & diffExp < 3)),
  nrow(filter(sceneData, isControlGroup == 1 & diffExp > 0 & diffExp < 4)),
  nrow(filter(sceneData, isControlGroup == 1 & diffExp > 1)),
  "testGroup",
  nrow(filter(sceneData, isControlGroup == 0 & diffExp < 3)),
  nrow(filter(sceneData, isControlGroup == 0 & diffExp > 0 & diffExp < 4)),
  nrow(filter(sceneData, isControlGroup == 0 & diffExp > 1))
)
...
subT  <- sprintf("ANN training. %d epochs. Trained on %d observations (scenes).", nrow(dataFrame), as.numeric(dataFrame[1,4]));
ggplot(dataFrame, aes(x=epoch)) +
  geom_smooth(aes(y = (correctGenExp/trainSize * 100), color = 'Geneneral'), fill = "brown", alpha = 0.3) + 
  geom_smooth(aes(y = (correctDiffExp/trainSize * 100), color = 'Difficulty'), fill = "yellow", alpha = 0.3) +
...
subT  <- sprintf("ANN training. %d epochs. Trained/Validated on %d/%d scenes.", nrow(dataFrame), as.numeric(dataFrame[1,4]), as.numeric(dataFrame[1,5]));
ggplot(dataFrame, aes(x=epoch)) +
  geom_smooth(aes(y = (correctGenExp/validateSize * 100), color = 'Geneneral'), fill = "brown", alpha = 0.3) + 
  geom_smooth(aes(y = (correctDiffExp/validateSize * 100), color = 'Difficulty'), fill = "yellow", alpha = 0.3) +
  geom_point(aes(y = (correctGenExp/validateSize * 100), color = 'Geneneral'), fill = "brown", alpha = 0.3, position = position_jitter(width = 0.15, height = 0.15)) + 
  geom_point(aes(y = (correctDiffExp/validateSize * 100), color = 'Difficulty'), fill = "yellow", alpha = 0.3, position = position_jitter(width = 0.15, height = 0.15)) +  
...
dataFrame <- sceneData
  ...
  scale_fill_manual(values = c("#5F9EA0", "#008B00", "#DEB887", "#EE6A50", "black"),
                    labels = c("I saw no adaptation", "I saw adaptation", "None of these", "I didn’t want adaptation", "I wanted more adaptation"),
                    drop = FALSE
                    ) +
